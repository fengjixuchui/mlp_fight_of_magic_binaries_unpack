...
def unpack(filename):
    ...
    os.mkdir(dirname)
    pak.seek(0x110, os.SEEK_SET)

    scenenum = readdw(pak)
    print("scenenum = " + str(scenenum))
    pak.seek(scenenum * 0x27, os.SEEK_CUR)

    posdatanum = readdw(pak)
    pak.seek(posdatanum * 0x10, os.SEEK_CUR)
    ...
    for index in range(imgnum):
        ...
        pak.seek(imgsize, os.SEEK_CUR)

    defpalette = pak.read(0x2100)
    pak.seek(offset, os.SEEK_SET)
